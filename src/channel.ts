...
import { cliAuthStatus } from "./basecamp-cli.js";
...
export const basecampChannel: ChannelPlugin<ResolvedBasecampAccount, BasecampProbe, BasecampAudit> = {
  ...
  auth: {
    login: async ({ cfg, accountId }) => {
      ...
      switch (account.tokenSource) {
        case "cli": {
          const { execCliAuthLogin } = await import("./basecamp-cli.js");
          await execCliAuthLogin({ profile: account.cliProfile });
          break;
        }
        case "oauth": {
  ...
  configSchema: {
    ...
    uiHints: {
      ...
      "accounts.*.cliProfile": { label: "Basecamp CLI profile", help: "Basecamp CLI profile name for auth" },
  ...
  config: {
    ...
    isConfigured: (account) => Boolean(account.token?.trim() || account.config.tokenFile || account.config.oauthTokenFile || account.cliProfile),
    isEnabled: (account) => account.enabled,
    disabledReason: () => "Manually disabled",
    unconfiguredReason: () => "No CLI profile, token, or OAuth token file configured",
    describeAccount: (account) => ({
      accountId: account.accountId,
      name: account.displayName,
      enabled: account.enabled,
      configured: Boolean(account.token?.trim() || account.config.tokenFile || account.config.oauthTokenFile || account.cliProfile),
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // Legacy: verify Basecamp CLI auth status for CLI accounts
      if (account.tokenSource === "cli" && account.cliProfile) {
        try {
          const authResult = await cliAuthStatus({ profile: account.cliProfile });
          if (!authResult.data.authenticated) {
            ctx.log?.error(
              `[${account.accountId}] cannot start: Basecamp CLI profile "${account.cliProfile}" is not authenticated`,
            );
            return;
          }
        } catch (err) {
          ctx.log?.error(
            `[${account.accountId}] cannot start: Basecamp CLI auth check failed for profile "${account.cliProfile}": ${String(err)}`,
          );
          return;
        }
      }
      ...
      if (!account.token && !account.cliProfile && account.tokenSource !== "oauth") {
        ctx.log?.error(
          `[${account.accountId}] cannot start: no token (check tokenFile or token config), no cliProfile, and no oauthTokenFile`,
        );
        return;
      }
