...
├── openclaw.plugin.json          # Plugin manifest (generated by `npm run manifest:gen`)
├── tsconfig.json
├── README.md
├── scripts/
│   └── install-local.sh          # Dev install helper
├── src/
│   ├── index.ts                  # defineChannelPluginEntry (full runtime entry)
│   ├── setup-entry.ts            # defineSetupPluginEntry (import-light setup entry)
│   ├── channel.ts                # createChatChannelPlugin composition
│   ├── channel-setup.ts          # Import-light subset: meta, config schema/adapter, setup contract, secrets
...
│   │   ├── replay-guard.ts       # Exactly-once claim/commit on persistent-dedupe (24h TTL)
│   │   └── recording-index.ts    # recording → bucket index (outbound targets, bindings)
│   ├── outbound/
...
```json
{
  ...
  "openclaw": {
    ...
    "channel": { "id": "basecamp", "aliases": ["bc"], "setup": { "fields": ["/* mirrors defineChannelSetupContract */"] } },
    "install": { "npmSpec": "@37signals/openclaw-basecamp", "minHostVersion": ">=2026.8.1" },
    "compat": { "pluginApi": ">=2026.8.1" },
    "build": { "openclawVersion": "2026.8.1", "pluginSdkVersion": "2026.8.1" }
  },
  "peerDependencies": { "openclaw": ">=2026.8.1" },
  "devDependencies": { "openclaw": "2026.8.1" }
}
```

`peerDependencies.openclaw` is what managed npm installs use to relink `node_modules/openclaw` against the host; `openclaw.compat.pluginApi` and `install.minHostVersion` are enforced at install and discovery. All three move together with each host bump (see RELEASING.md).

**`openclaw.plugin.json`** (generated — `npm run manifest:gen`; `tests/manifest-contract.test.ts` fails when the checked-in file drifts from the code):
```json5
{
  "id": "basecamp",                                   // plugin id == channel id (Q1 of the 2.0 spec)
  "name": "Basecamp", "version": "0.1.0",             // version kept in lockstep with package.json
  "channels": ["basecamp"],
  "activation": { "onStartup": false, "onChannels": ["basecamp"] },
  "configSchema": { "type": "object", "additionalProperties": false, "properties": {} },
  "contracts": { "tools": ["basecamp_create_todo", "..."] },   // consent surface for api.registerTool
  "toolMetadata": { "basecamp_read_history": { "replaySafe": true }, "basecamp_post_message": { "sideEffecting": true } },
  "doctorContract": { "configRepair": true },
  "backupResources": [{ "disposition": "include", "scope": "state", "relativePath": "plugins/basecamp" }],
  "channelConfigs": { "basecamp": { "schema": { /* from BasecampConfigSchema */ }, "uiHints": { /* from channel-setup.ts */ } } }
}
```

**Note on config schema:** The Zod schema in `src/config.ts` is the single source of truth (see Q11). `ChannelPlugin.configSchema` is built from it with `buildChannelConfigSchema`, and the manifest's `channelConfigs.basecamp.{schema,uiHints}` is the same object projected to JSON Schema so the host can validate `channels.basecamp` in setup, Control UI, and `openclaw doctor` without loading the runtime. The manifest's top-level `configSchema` (for `plugins.entries.basecamp.config`) is empty — the plugin has no plugin-scoped config.
...
export default defineChannelPluginEntry({
  ...
  setRuntime: setBasecampRuntime,          // runs before registerFull in "full" mode only
  registerFull(api) {                      // also runs in "tool-discovery" mode — no runtime access here
    api.registerHttpRoute({ path: "/webhooks/basecamp", auth: "plugin", handler: handleBasecampWebhook });
    api.on("gateway_stop", flushBasecampState);
    registerBasecampTools(api);            // api.registerTool factories (see Agent tools)
  },
});
```
...
**`src/channel.ts`:** (shape)
```typescript
import { createChannelPluginBase, createChatChannelPlugin } from "openclaw/plugin-sdk/channel-core";

const base = {
  ...createChannelPluginBase<ResolvedBasecampAccount>({ id: "basecamp", meta, setupWizard, agentPrompt, configSchema, security, setupContract, doctor, groups, reload: { configPrefixes: ["channels.basecamp"] } }),
  capabilities, config, secrets, status, directory, messaging, resolver, heartbeat, lifecycle, mentions, actions, allowlist,
  conversationBindings: { supportsCurrentConversationBinding: false },   // bindings are project/recording-level
  auth: { login },                                                       // OAuth browser login
  gateway: { startAccount, stopAccount, logoutAccount },                 // poller + webhook reconciliation
};

export const basecampChannel = createChatChannelPlugin({
  base,
  pairing,                                              // text pairing; codes delivered by Ping
  outbound: {
    base: { deliveryMode: "direct", chunkerMode: "markdown", textChunkLimit: 10_000, presentationCapabilities, deliveryCapabilities: { durableFinal: { text: true } }, renderPresentation, resolveTarget },
    attachedResults: { channel: "basecamp", sendText, sendMedia },
  },
});
```
...
```
`--force` confirms a non-catalog source (the package is not in OpenClaw's official catalog); `--accept-capabilities` consents to the declared channel + tools (re-asked on updates that widen the surface). If the config has a `plugins.allow` list it must contain the plugin id `basecamp`, not the package name.

**For development (local checkout — requires build):**
```bash
...
```
`--link` creates a managed install record (consent tracked) but loads from the checkout. The alternative is a bare load path with no install record:
```yaml
# openclaw.yaml
plugins:
  load:
    paths:
      - ~/Work/basecamp/openclaw-basecamp
```

**Pack-install proof (CI and release):** `npm pack`, then `openclaw plugins install npm-pack:<tgz> --force --accept-capabilities` and `openclaw plugins inspect basecamp --runtime --json` in a scratch `OPENCLAW_STATE_DIR`.
...
**Primary:** Basecamp account as `accountId`. `channels.basecamp.accounts.<accountId>` holds auth + account info. Per-project routing comes from `bucket:<id>` peer bindings (matched through the recording→bucket index).

**Optional:** Virtual bucket aliases. `channels.basecamp.virtualAccounts.<alias>` → `{ accountId, bucketId }`. If a bucketId matches a virtual alias, inbound events emit `accountId = <alias>` for clean per-project bindings without a peer match. Teams choose their preferred style:

```jsonc
// Style A: bucket peer binding (minimal config)
...
**Replay-guard rules** (`src/inbound/replay-guard.ts`, see Q18):
- Primary key: source-prefixed event id (`activity:<id>`, `reading:<id>`, `webhook:<id>`)
- Secondary key: `recordingId:action:createdAt` — collapses the same Basecamp event seen from two sources
- A claim succeeds only when *every* key is new; keys are committed after the agent dispatch succeeds and released on failure, so a later source can retry a failed event
- Per-account namespace, 24h TTL; `hasRecent` serves the safety-net and reconciliation passes
...
```typescript
{
  ...
  parentPeer?: {
    kind: "group",
    id: string,                            // bucket:<bucketId>; routing itself resolves the parent via resolveSessionConversation
  },
  sender: {
...
Priority 2: bucket match (specific project)
  { channel: "basecamp", peer: { kind: "group", id: "bucket:123" } }
  → the bindings adapter matches any recording whose index entry names bucket 123
    (parentConversationCandidates from messaging.resolveSessionConversation)
...
### Q11: Config Schema Location → **Code is the source; the manifest `channelConfigs` is generated from it**

The Zod schema in `src/config.ts` (built on the SDK's `buildChannelAccountSchemaParts`, with SecretRef leaves for `token`, `oauthClientSecret`, `webhookSecret`, `oauth.clientSecret`) is the single source of truth. `ChannelPlugin.configSchema` wraps it with `buildChannelConfigSchema` + uiHints, and `scripts/generate-manifest.ts` (`npm run manifest:gen`) projects the same schema and hints into `openclaw.plugin.json#channelConfigs.basecamp` so the host can validate and render `channels.basecamp` without loading the runtime. `tests/manifest-contract.test.ts` regenerates in-process and fails on drift. The manifest's top-level `configSchema` stays empty (no `plugins.entries.basecamp.config`).
...
### Q18: Dedup TTL & Storage Layout → **Replay guard on `persistent-dedupe`, 24h TTL; plugin-owned JSON under the state dir**

Exactly-once processing is a plugin-owned `ReplayGuard` seam (`src/inbound/replay-guard.ts`) backed by `openclaw/plugin-sdk/persistent-dedupe`'s claimable dedupe: multi-key claim (primary + cross-source secondary), commit after dispatch succeeds / release on failure, per-account namespace, 24h TTL. Committed keys persist in OpenClaw's shared plugin-state SQLite store, so the plugin no longer ships its own SQLite or `node:sqlite` dependency. The seam is intentionally thin so a durable-ingress backend can slot in behind it later.

Why not the host's durable ingress queue or keyed stores: in 2026.8.1 `api.runtime.state.{openKeyedStore, openBlobStore, openChannelIngressQueue, openChannelIngressDrain}` and `createChannelIngressMonitor` throw unless the plugin is bundled or in OpenClaw's bundled official-catalog — a trust gate, not a capability declaration. Until the plugin is catalogued (docs/upgrades/official-catalog-nomination.md) webhook delivery stays at-most-once behind a per-account in-flight limiter, and everything else is plugin-owned files written with the SDK's atomic `json-store` helpers:

```
<stateDir>/plugins/basecamp/                # runtime.state.resolveStateDir() + plugins/basecamp
  ├── cursors-<accountId>.json              # Polling cursors (activity feed position, readings cursor, assignment set)
  ├── recording-index-<accountId>.json      # recording → { bucketId, recordableType } (outbound targets, bindings)
  ├── webhook-secrets-<accountId>.json      # Per-project webhook HMAC secrets
  └── tokens/<accountId>.json               # OAuth tokens (+ companion .client.json)
```

The directory is declared in the manifest's `backupResources`, so `openclaw backup` includes it; replay-guard state needs no separate backup. OAuth tokens moved here from `~/.local/share/openclaw/basecamp/tokens/` (one-time courtesy copy on first read).
...
### Q21: Outbound Target Grammar → **Peer ids stay as-is; recording→bucket index resolves the rest**

Inbound peer ids are unchanged (`recording:<id>` / `bucket:<id>` / `ping:<id>`), so sessions and bindings keep their identity. Outbound (`sendText`/`sendMedia` via `createChatChannelPlugin({ outbound: { base, attachedResults } })`) accepts the same grammar, optionally prefixed `basecamp:`/`bc:` (`messaging.targetPrefixes`):

| Target | Resolution |
|--------|------------|
| `recording:<id>` | Look up `{ bucketId, recordableType }` in the recording index; post a Campfire line (transcripts) or comment (everything else) |
| `bucket:<id>` | Campfire line in the project's default chat (dock lookup, cached) |
| `bucket:<b>/recording:<r>` | Cold send: comment on a recording the index has never seen |
| `ping:<id>` | Line in the Circle chat; id = circle bucket id |

An index miss on a bare `recording:` target throws an error naming the explicit form. Results are `{ messageId, target: { kind: "conversation", id } }`; not-sent failures throw. Each successful post is recorded with `recordOutboundMessageIdentity` so the inbound path can suppress echoes of our own posts in addition to the `personId` self-filter. This unlocks the shared `message` tool, `openclaw message send --channel basecamp --target …`, cron/heartbeat announcements, and approval / credential-link delivery.

### Q22: Ingress Authorization & Pairing → **SDK ingress resolver replaces the hand-rolled DM gate**

`dispatch.ts` runs each event through `createChannelIngressResolver` with a stable identity keyed on the Basecamp person id. DM policy (`dmPolicy`, per-account overrides via `buildChannelAccountSchemaParts`), the pairing store, access groups, `groupPolicy`, and per-bucket `allowFrom` (a `{ id: "bucket", senderAllowFrom, senderPolicy: "replace" }` route descriptor) are all evaluated by the resolver; the plugin keeps only the `engage` gate and the self/echo filter. `dmPolicy: "pairing"` is real: unknown senders receive a pairing code by Ping (`createChannelPairingChallengeIssuer` + `POST /circles/people/<id>/lines.json`), `openclaw pairing approve` unblocks them, and the `allowlist` adapter lets `openclaw allowlist` / `openclaw pairing` edit the config-backed lists. Default policy when unset is `allowlist`.
...
Durable ingress queues/monitors and keyed/blob stores are trust-gated to bundled and official-catalog plugins (see Q18). Webhook handling therefore uses `readJsonWebhookBodyOrReject`, `createWebhookInFlightLimiter`, and `runDetachedWebhookWork` (bounded, drain-aware, at-most-once) with HMAC verification plugin-owned; state uses `resolveStateDir` + `json-store`. Catalog nomination is tracked in docs/upgrades/official-catalog-nomination.md; landing it swaps the backends without changing callers.
...
│   └── ...
├── openclaw.plugin.json          Plugin manifest (channelConfigs generated from the code schema)
...
- bucket peer bindings for project-level routing (recording index + resolveSessionConversation)
...
| `openclaw.plugin.json` | basecamp-openclaw-plugin | New | Plugin manifest (generated; channelConfigs from the code schema) |
...
| `src/inbound/replay-guard.ts` | basecamp-openclaw-plugin | New | Exactly-once replay guard (persistent-dedupe) |
| `src/inbound/recording-index.ts` | basecamp-openclaw-plugin | New | recording → bucket index for outbound targets and bindings |
